...
export default function YieldBox() {
  ...
  sessionStorage.setItem('constructedFieldImage', '/images/field-image.png'); // Assuming this is the path
  setLoading(true);
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] bg-gray-100 rounded-xl px-6 py-6 text-center">
        ...
        <p className="text-lg font-semibold text-gray-800">{statusTexts[statusIndex]}</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col items-center justify-center px-6 pt-6 pb-12">
      ...
      <div className="w-full max-w-2xl bg-white border border-dashed border-[#64FF64] rounded-xl p-6 text-center transition shadow-sm hover:shadow-lg group">
        ...
        >
          ...
          <p className="text-sm text-gray-500">(Multiple JPG, JPEG, PNG)</p>
        </label>
          ...
          <div className="mt-6 grid grid-cols-2 sm:grid-cols-3 gap-4">
            {images.map((src, index) => (
              <img
                ...
                src={src}
                alt={`Preview ${index + 1}`}
                className="rounded-lg h-32 w-full object-cover border border-gray-300 shadow-sm"
              />
            ))}
          </div>
        )}
      ...
      {images.length > 0 && !showFieldImage && (
        <button
          ...
          className="cursor-pointer mt-6 px-5 py-2 bg-black text-white border border-[#64FF64] rounded-lg hover:bg-[#1a1a1a] transition"
        >
